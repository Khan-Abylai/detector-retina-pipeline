...
img_path = "curve/uae_test.jpeg"
# Load the TFLite model and allocate tensors
interpreter = tf.lite.Interpreter(model_path='weights/PlateDetector-mobilenet_float16.tflite')
...
scl_w= orig_width / 480
scl_h= orig_height / 480
...
for b in dets:
    ...
    text = "{:.4f}".format(b[4])
    b = list(map(int, b))
    cv2.rectangle(img_orig, ( int(b[0]*scl_w), int(b[1]*scl_h)), (int(b[2]*scl_w), int(b[3]*scl_h)), (0, 0, 255), 2)
    cx = int(b[0]*scl_w)
    cy = int(b[1]*scl_h) - 15
    cv2.putText(img_orig, text, (cx, cy),
                cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 255, 255))
    # landms
    cv2.circle(img_orig, (int(b[5]*scl_w), int(b[6]*scl_h)), 1, (0, 0, 255), 4)
    cv2.circle(img_orig, (int(b[7]*scl_w), int(b[8]*scl_h)), 1, (0, 255, 255), 4)
    cv2.circle(img_orig, (int(b[9]*scl_w), int(b[10]*scl_h)), 1, (255, 0, 255), 4)
    cv2.circle(img_orig, (int(b[11]*scl_w), int(b[12]*scl_h)), 1, (0, 255, 0), 4)
    cv2.circle(img_orig, (int(b[13]*scl_w), int(b[14]*scl_h)), 1, (255, 0, 0), 4)

cv2.imwrite("results/" + img_path.split("/")[1].split(".")[0] + "_mobilenet_tflite_orig.jpg", img_orig)